...
/**
 * useRegistrationFormFieldsStore
 *
 * 管理報名表欄位（RegistrationFormField）
 *
 * API endpoints:
 *   GET  /api/registration-form-fields/by_page/{pageId}/   → 取得欄位清單（含隱藏）
 *   POST /api/registration-form-fields/bulk_save/          → 整張欄位一次儲存
 *
 * bulk_save 請求格式:
 *   {
 ...
 *         id: null | number,
 *         label: string,
 *         field_type: 'text' | 'tel' | 'email' | 'select' | 'radio' | 'textarea',
 *         is_required: boolean,
 *         is_hidden: boolean,
 *         is_fixed: boolean,
 *         order: number,
...
export const useRegistrationFormFieldsStore = defineStore('registrationFormFields', () => {
     ...
     * 取得指定報名頁的所有欄位（含隱藏）
     */
    async function fetchFields(pageId) {
        loading.value = true
        clearError()
        try {
     ...
     * 後端自動處理新增/更新/刪除，is_fixed=true 欄位不會被刪除
     */
    async function bulkSave(pageId, fieldList) {
        saving.value = true
        clearError()
        try {
            const payload = {
